...
async function responseText(path, maxBytes) {
  const url = new URL(path, ACTIVITY_REPOSITORY.baseUrl);
  const base = new URL(ACTIVITY_REPOSITORY.baseUrl);
...
async function responseBytes(path, maxBytes) {
  const url = new URL(path, ACTIVITY_REPOSITORY.baseUrl);
  const base = new URL(ACTIVITY_REPOSITORY.baseUrl);
  if (url.origin !== base.origin || !url.pathname.startsWith('/ChudForks/ChudPresence-Activities/')) {
    throw new Error('Activity repository path is outside the configured repository.');
...
export async function fetchCatalog({ force = false } = {}) {
  ...
  if (!force && cached.catalog) return { ...cached, fromCache: true };
  const text = await responseText(ACTIVITY_REPOSITORY.catalogPath, MAX_CATALOG_BYTES);
  let catalog;
  try {
...
export async function downloadActivity(activityId, catalog) {
  ...
  const [metadataText, source, iconBytes] = await Promise.all([
    responseText(entry.metadata, MAX_METADATA_BYTES),
    responseText(entry.entry, MAX_ACTIVITY_SOURCE_BYTES),
    entry.icon ? responseBytes(entry.icon, MAX_ICON_BYTES) : Promise.resolve(null),
  ]);
  ...
  let metadata;
  try {
    metadata = validateActivityMetadata(JSON.parse(metadataText));
  } catch (error) {
  ...
  return { metadata, source, ...(icon ? { icon } : {}), sourceType: 'repository' };
}
